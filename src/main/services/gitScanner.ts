...
export class GitScanner {
  ...
  private async scanDirectoryForGitRepos(dirPath: string, maxDepth: number = 3): Promise<GitRepository[]> {
  ...
  async scanRepositories(rootPath: string): Promise<ScanResult> {
    ...
    try {
      ...
      console.log('仓库列表:', repositories.map(r => ({ name: r.name, path: r.path, isValid: r.isValid })));
